...
public class Mine implements ConfigurationSerializable, IMine {
    ...
    public static final String PLAYER_NAME = "MineResetLite-Player-";
    ...
    private File[] m_schematicFiles = new File[0];
...
//		this.holograms = new LinkedList();
    ...
    public Mine(Map<String, Object> me) {
        ...
        if (me.containsKey("schematic")) {
            setSchematic((String) me.get("schematic"));
        }
...
////		me.put("holograms", holograms);
     ...
     * Map<Location, String[]> m = new HashMap<Location, String[]>(); //	for
     * (String s : holograms) { //	String loc = s.split("#")[0]; //	Location l =
     * new Location(world, Integer.parseInt(loc.split(",")[0]),
     * Integer.parseInt(loc.split(",")[1]),
     * Integer.parseInt(loc.split(",")[2])); //	String data = s.split("#")[1];
     * //	String[] lines = data.split("\n"); //	m.put(l, lines); //	} //	return
     * m; //	}
     */
...
//		if (MineResetLite.instance.resetting) return; // Disabled for now
        ...
        final String jobName = name + "-" + UUID.randomUUID().toString();

        final PlayerEntry pEntry = new PlayerEntry(PLAYER_NAME + UUID.randomUUID().toString(), UUID.randomUUID());
        final ThreadSafeEditSession es = m_esFactory.getThreadSafeEditSession(new BukkitWorld(world), 99999999, null, pEntry);
        ...
        FuncParamEx<Integer, CancelabeEditSession, MaxChangedBlocksException> function;
        if (m_schematicFiles.length > 0) {
            final File f;

            if (m_schematicFiles.length > 1) {
                List<File> available = new ArrayList<File>();
                for (File sf : m_schematicFiles) {
                    if (!sf.equals(last)) {
                        available.add(sf);
                    }
                }

                f = available.get(s_rnd.nextInt(available.size()));
            } else {
                f = m_schematicFiles[0];
            }

            last = f;

            if (!f.exists()) {
                MineResetLite.broadcast("Error Schematic not found... " + f.getName(), this);
                return;
            }

            MineResetLite.broadcast(Phrases.phrase("schematicBc", this, f.getName()), this);

            function = new MineResetSchematic(m_blockPlacer, name, f, this, origin, minX, maxY, minZ);
        } else {
            function = new MineResetManual(m_blockPlacer, name, this, world, new MinePattern(this), minX, minY, minZ, maxX, maxY, maxZ);
        }

        m_blockPlacer.performAsAsyncJob(es, pEntry, jobName, function);
    }
     ...
     * double blocksSeen = 0.0D; //	for (int x = this.minX; x <= this.maxX; x++)
     * { //	for (int y = this.minY; y <= this.maxY; y++) { //	for (int z =
     * this.minZ; z <= this.maxZ; z++) { //	blocksSeen += 1.0D; //	if
     * (blocks.containsKey(Integer.valueOf(this.world.getBlockAt(x, y,
     * z).getTypeId()))) //	blocks.put(Integer.valueOf(this.world.getBlockAt(x,
     * y, z).getTypeId()), Integer.valueOf(((Integer)
     * blocks.get(Integer.valueOf(this.world.getBlockAt(x, y,
     * z).getTypeId()))).intValue() + 1)); //	else if
     * (!blocks.containsKey(Integer.valueOf(this.world.getBlockAt(x, y,
     * z).getTypeId()))) { //
     * blocks.put(Integer.valueOf(this.world.getBlockAt(x, y, z).getTypeId()),
     * Integer.valueOf(1)); //	} //	} //	} //	} //	// Because some armadillo
     * says so, we're adding the functionality to negate air blocks already in
     * it. isn't that great. //	if (blocks.containsKey(0)) { //	if (air != 0) {
     * //	blocks.put(0, blocks.get(0) - air); //	} //	} //	Map<Integer, Integer>
     * percentageComposition = new HashMap<Integer, Integer>(); //	for
     * (Map.Entry<Integer, Integer> entry : blocks.entrySet()) { //
     * percentageComposition.put(entry.getKey(), (int)
     * (Double.valueOf(entry.getValue().intValue() / blocksSeen) * 100D)); //	}
    ...
    public void cron2() {
        ...
        if (!isDone()) {
            if (total != 0) {
                int left = 0;
                for (PlayerEntry pEntry : m_blockPlacer.getAllPlayers()) {
                    if (!pEntry.getName().startsWith(PLAYER_NAME)) {
                        continue;
                    }
                    
                    BlockPlacerPlayer p = m_blockPlacer.getPlayerEvents(pEntry);
                    if (p != null) {
...
//							System.out.println(left + " blocks left.");
    ...
    public void setSchematic(String schematic) {
        ...
        if (schematic == null) {
            m_schematicFiles = new File[0];
            return;
        }

        List<File> result = new ArrayList<File>();
        for (String sf : schematic.split(",")) {
            final File f = new File("plugins/WorldEdit/schematics/" + sf + ".schematic");
            if (f.exists() && f.canRead()) {
                result.add(f);
            }
        }

        m_schematicFiles = result.toArray(new File[0]);
    }
