...
public class MineResetLite extends JavaPlugin {
	...
	private class SignClicker implements Listener {
		@EventHandler
		public void onSign(SignChangeEvent e) {
			if (e.getLine(0).equalsIgnoreCase("[ResetMine]") && e.getPlayer().isOp()) {
				// Woo
				e.setLine(0, ChatColor.DARK_BLUE + e.getLine(0));
				String s = e.getLine(2);
				if (MineResetLite.instance.matchMines(s).length == 0) {
					e.setCancelled(true);
					e.getPlayer().sendMessage(ChatColor.RED + "There is not a mine called that, (Line 3)");
				}
			}
		}
		...
		public void onInteract(PlayerInteractEvent e) {
			...
			if (e.getAction() == Action.RIGHT_CLICK_BLOCK || e.getAction() == Action.LEFT_CLICK_BLOCK) {
				if (e.getClickedBlock().getState() instanceof Sign) {
					if (e.getPlayer().isOp() && e.getPlayer().isSneaking() && e.getAction() == Action.LEFT_CLICK_BLOCK) {
						return;
					}
					Sign s = (Sign) e.getClickedBlock().getState();
					if (s.getLine(0).equalsIgnoreCase(ChatColor.DARK_BLUE + "[ResetMine]")) {
						e.setCancelled(true);
						String x = s.getLine(2);
						if (MineResetLite.instance.matchMines(x).length == 0) {
							e.setCancelled(true);
							e.getPlayer().sendMessage(ChatColor.RED + "There is not a mine called that, (Line 3) :(");
						} else {
							Mine m = MineResetLite.instance.matchMines(x)[0];
							int i = m.attemptReset(e.getPlayer());
							if (i == 0) {
								e.getPlayer().sendMessage(ChatColor.RED + "It was a success! You have reset the mine!");
							} else {
								e.getPlayer().sendMessage(ChatColor.RED + "This mine was recently reset, try again in " + i + " seconds.");
							}
						}
					}
				}
	...
	public void onEnable() {
		...
		async.getBlockPlacer().addListener(new MineListener());
...
//		}
		...
		getServer().getPluginManager().registerEvents(new SignClicker(), this);
...
//            int localVer = Integer.valueOf("0.0.1");
...
//        }
	...
	public void onDisable() {
		...
		for(Mine m:mines){
			if (m.hologram != null) {
				if (m.replace != null) {
					m.replace.getVisibilityManager().setVisibleByDefault(true);
				}
				m.hologram.delete();
				m.hologram = null;
			}
	...
	public static void broadcast(String message, Mine mine) {
		if (Config.getBroadcastNearbyOnly()) {
			for (Player p : mine.getWorld().getPlayers()) {
				if (mine.isInside(p) || mine.everyone) {
